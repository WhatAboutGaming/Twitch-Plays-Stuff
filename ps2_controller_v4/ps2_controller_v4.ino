...
unsigned long ps2FrameCount = 0;

unsigned long ps2FramePeriodMillis = 0;

unsigned long ps2FrameMillis = 0;
unsigned long ps2FrameMillisPrevious = 0;

unsigned long ps2FramesThisSecond = 0;

unsigned long secondsElapsed = 0;
unsigned long secondsElapsedPrevious = 0;
...
void loop() {
  if (Serial.available() > 0) {
    ...
    if ((serial_rx_buffer[0] == 0x01) && (serial_rx_buffer[11] == 0x01)) {
      ...
      previousInputDelay = ps2FrameCount;
    }
...
void readAttentionPin() {
  ...
  if (attentionLevel != attentionLevelPrevious) {
    if (attentionLevel == LOW) {
      ps2FrameMillis = millis();
      ps2FrameCount++;
      ps2FramesThisSecond++;
      ps2FramePeriodMillis = ps2FrameMillis - ps2FrameMillisPrevious;
  ...
  if (isInputting == true) {
    ...
    if (inputDelay != 0) {
      ...
      if (isInputtingDelayed == true) {
        if (ps2FrameCount - previousInputDelay >= inputDelay) {
